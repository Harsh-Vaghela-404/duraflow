...
| **Step** | An idempotent sub-unit of a workflow, declared inside the handler via `step.run(name, fn, opts)`. Identified by `(task_id, step_key)`. Memoized — re-runs return the cached output. May declare a compensation. | `StepRunsEntity`; row in `step_runs` table; one `step.run(...)` call | `apps/engine/src/db/step_runs.entity.ts`, `apps/engine/src/workers/step-worker.ts:124-173` |
| **Saga** | A workflow becomes a saga the moment it declares a `compensations` map. When the workflow fails (or is cancelled), compensations run in LIFO order over the completed steps. | `RollbackOrchestrator` is the runtime; the `compensations` map on `workflow(name, handler, { compensations })` is the author-side declaration | `apps/engine/src/services/rollback-orchestrator.ts`, `packages/sdk/src/workflow.ts`, `packages/sdk/src/types.ts` |
| **Compensation** | The undo function for a completed step. **Pure function of the step's saved output** (no closure over handler state). Registered by string name (`${workflowName}:${stepKey}`) at module load, so it resolves in every process that loads the workflow. Should be idempotent — a rollback retry may re-invoke it. | `(output: unknown) => Promise<void>`; entries in `compensationRegistry` (`Map<string, Fn>`) | `packages/sdk/src/compensation.ts`, `packages/sdk/src/workflow.ts` |
...
| **Leader election** | Redis-backed distributed lock that ensures only one process runs the reaper at a time. Key: `duraflow:reaper:leader`. Acquired via `SET NX EX`, renewed via Lua `EXPIRE`. | `LeaderElector` class | `apps/engine/src/services/leader-elector.ts` |
...
| **IPC** | Inter-thread communication between the main thread and Piscina worker. Uses `MessageChannel` / `MessagePort`. Each request has a UUID id and a 30s timeout. | `IPCClient` class in the worker, `handleWorkerMessage` in the executor | `apps/engine/src/workers/step-worker.ts:65-122`, `apps/engine/src/services/workflow-executor.ts:67-94,110-147` |
...
- **Compensation key is `${workflowName}:${stepKey}`** — Not just `stepKey`. See `step-worker.ts:143`.
...
- **`registerCompensation` is called once at workflow file load** in the worker (`step-worker.ts:24-35`). For tests, register in `beforeEach`.
