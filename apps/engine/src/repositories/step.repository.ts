...
export class StepRepository {
  ...
  async findByTaskAndKey(
    taskId: string,
    stepKey: string,
  ): Promise<StepRunsEntity | null> {
  ...
  async findCompletedWithCompensation(
    taskId: string,
  ): Promise<StepRunsEntity[]> {
    const res = await this.pool.query(
  ...
  async findById(id: string): Promise<StepRunsEntity | null> {
    const res = await this.pool.query("SELECT * FROM step_runs WHERE id = $1", [
      id,
    ]);
    return res.rows[0] || null;
  }
