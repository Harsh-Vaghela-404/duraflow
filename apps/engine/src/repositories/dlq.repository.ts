...
export class DeadLetterQueueRepository {
    ...
    async findById(id: string): Promise<DeadLetterQueueEntity | null> {
        const res = await this.pool.query('SELECT * FROM dead_letter_queue WHERE id = $1', [id]);
    ...
    async countAll(): Promise<number> {
        const res = await this.pool.query('SELECT COUNT(*) as count FROM dead_letter_queue');
