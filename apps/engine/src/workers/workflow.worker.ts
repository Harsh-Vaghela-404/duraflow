...
for (const p of workflowPaths) {
    try {
        ...
        console.error(`[worker] Failed to load workflow: ${p}`, err);
    }
}

type IPCMessageType = 'STEP_FIND' | 'STEP_CREATE_OR_FIND' | 'STEP_COMPLETE' | 'STEP_FAIL' | 'STEP_INCREMENT';
    ...
    return {
        async run<T>(name: string, fn: () => Promise<T>, opts?: StepOptions): Promise<T> {
            ...
            try {
                ...
                await ipc.send('STEP_COMPLETE', { stepId: step.id, output: JSON.parse(serialize(result)) });
                return result;
            } catch (err) {
    ...
    try {
        ...
        if (err instanceof StepRetryError) {
            throw {
                ...
                delay: err.delay,
                attempt: err.attempt,
                originalError: err.originalError instanceof Error
