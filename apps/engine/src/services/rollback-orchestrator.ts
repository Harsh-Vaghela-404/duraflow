...
export class RollbackOrchestrator {
    ...
    constructor(pool: Pool, options?: { compensationTimeoutMs?: number }) {
        ...
        this.defaultTimeout = options?.compensationTimeoutMs ?? DEFAULT_COMPENSATION_TIMEOUT_MS;
    }

    async rollback(taskId: string, options?: RollbackOptions): Promise<RollbackResult> {
        ...
        for (const step of steps) {
            ...
            try {
                ...
                const isTimeout = err instanceof Error && err.message === 'Compensation timeout';
                const errorContext = {
                    taskId,
                    stepId: step.id,
                    stepKey: step.step_key,
