...
export class WorkflowExecutor {
    ...
    async execute(task: TaskEntity): Promise<unknown> {
        ...
        try {
            ...
            if (freshTask?.status === taskStatus.CANCELLED) {
                console.log(
                    `${TAG} task ${task.id} cancel landed after worker finished - triggering rollback`,
                );
                try {
                    await this.rollback.rollback(task.id);
                } catch (rollbackErr) {
                    console.error(
                        `${TAG} post-cancel rollback failed for task ${task.id}:`,
                        rollbackErr,
                    );
                }
                return;
            ...
            if (this.isStepRetry(err)) {
                console.log(
                    `${TAG} task ${task.id} retry scheduled in ${err.delay}ms (attempt ${err.attempt})`,
                );
                await this.taskRepo.scheduleRetry(
                    task.id,
                    err.delay,
                    err.attempt,
                    err.originalError,
            ...
            if (this.isAbortError(err)) {
                console.log(
                    `${TAG} task ${task.id} aborted by cancel signal - triggering rollback`,
                );
                try {
                    await this.rollback.rollback(task.id);
                } catch (rollbackErr) {
                    console.error(
                        `${TAG} post-abort rollback failed for task ${task.id}:`,
                        rollbackErr,
                    );
                }
                return;
    ...
    private async handleWorkerMessage(request: IPCRequest): Promise<IPCResponse> {
        try {
            switch (request.type) {
                case 'STEP_FIND': {
                    const { taskId, stepKey } = request.payload as {
                        taskId: string;
                        stepKey: string;
                    };
                ...
                case 'STEP_COMPLETE': {
                    ...
                    await this.stepRepo.updateCompleted(
                        stepId,
                        output,
                        compensationFn ?? undefined,
                    );
                ...
                case 'RATE_LIMIT_ACQUIRE': {
                    ...
                    try {
                        ...
                        console.warn(
                            `${TAG} rate limiter error for API '${api}', allowing through:`,
                            rlErr,
                        );
                        return { id: request.id, success: true, data: true };
                    }
                ...
                default:
                    return {
                        id: request.id,
                        success: false,
                        error: { message: 'Unknown IPC message type', name: 'Error' },
                    };
            }
        } catch (err) {
            const error =
