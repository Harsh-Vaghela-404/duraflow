...
const PROTO_PATH = path.resolve(__dirname, '../../../../packages/proto/agent.service.proto');
...
function createClient(port: number): any {
    return new agentProto.AgentService(`localhost:${port}`, grpc.credentials.createInsecure());
}
    ...
    beforeAll(async () => {
        ...
        engine = spawn('npx', ['tsx', enginePath], {
            env: {
                ...process.env,
                PORT: String(PORT),
                DURAFLOW_WORKFLOWS: path.resolve(__dirname, '../workflows/test-workflows.ts'),
                DATABASE_URL:
